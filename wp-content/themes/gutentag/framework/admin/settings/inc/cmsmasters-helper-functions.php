...
function gutentag_get_admin_page() {
	global $pagenow;
...
function gutentag_default_tab() {
	$current_page = gutentag_get_admin_page();
...
function gutentag_get_the_tab() {
	global $pagenow;
	
	$default_tab = gutentag_default_tab();
...
function gutentag_settings_page_header() {
	...
	if ($tabs != '') {
		...
		if (class_exists('Cmsmasters_Custom_Fonts') && $page == 'cmsmasters-settings-font') {
			$links[] = '<a class="' . $class . ' cmsmasters_recaptcha_nav_tab' . '" href="edit.php?post_type=cmsmasters_font">' . esc_html__('Custom Fonts', 'gutentag') . '</a>';
		}
