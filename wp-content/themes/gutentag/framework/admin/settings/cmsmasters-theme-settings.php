...
add_action('admin_menu', 'gutentag_add_menu');
add_action('admin_init', 'gutentag_register_settings');
...
function gutentag_get_settings() {
	$output = array();
	
	
	$page = gutentag_get_admin_page();
	...
	case 'cmsmasters-settings':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'gutentag' . CMSMASTERS_THEME_STYLE;
		$cmsmasters_settings_page_title = esc_html__('Theme General Settings', 'gutentag');
		$cmsmasters_page_sections = gutentag_options_general_sections();
		$cmsmasters_page_fields = gutentag_options_general_fields();
		$cmsmasters_page_tabs = gutentag_options_general_tabs();
	...
	case 'cmsmasters-settings' . '-font':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'gutentag' . CMSMASTERS_THEME_STYLE . '_font';
		$cmsmasters_settings_page_title = esc_html__('Theme Fonts Settings', 'gutentag');
		$cmsmasters_page_sections = gutentag_options_font_sections();
		$cmsmasters_page_fields = gutentag_options_font_fields();
		$cmsmasters_page_tabs = gutentag_options_font_tabs();
	...
	case 'cmsmasters-settings' . '-color':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'gutentag' . CMSMASTERS_THEME_STYLE . '_color';
		$cmsmasters_settings_page_title = esc_html__('Theme Color Schemes', 'gutentag');
		$cmsmasters_page_sections = gutentag_options_color_sections();
		$cmsmasters_page_fields = gutentag_options_color_fields();
		$cmsmasters_page_tabs = gutentag_options_color_tabs();
	...
	case 'cmsmasters-settings' . '-element':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'gutentag' . CMSMASTERS_THEME_STYLE . '_element';
		$cmsmasters_settings_page_title = esc_html__('Theme Elements Settings', 'gutentag');
		$cmsmasters_page_sections = gutentag_options_element_sections();
		$cmsmasters_page_fields = gutentag_options_element_fields();
		$cmsmasters_page_tabs = gutentag_options_element_tabs();
	...
	case 'cmsmasters-settings' . '-single':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'gutentag' . CMSMASTERS_THEME_STYLE . '_single';
		$cmsmasters_settings_page_title = esc_html__('Theme Single Posts Settings', 'gutentag');
		$cmsmasters_page_sections = gutentag_options_single_sections();
		$cmsmasters_page_fields = gutentag_options_single_fields();
		$cmsmasters_page_tabs = gutentag_options_single_tabs();
	...
	case 'cmsmasters-settings' . '-demo':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'gutentag' . CMSMASTERS_THEME_STYLE . '_demo';
		$cmsmasters_settings_page_title = esc_html__('Theme Settings Import/Export', 'gutentag');
		$cmsmasters_page_sections = gutentag_options_demo_sections();
		$cmsmasters_page_fields = gutentag_options_demo_fields();
		$cmsmasters_page_tabs = gutentag_options_demo_tabs();
...
function gutentag_create_settings_field($args = array()) {
	$defaults = array( 
		'id' => 		'default_field', 
		'title' => 		esc_html__('Default Field', 'gutentag'), 
		'desc' => 		esc_html__('This is a default description.', 'gutentag'), 
		'std' => 		'', 
		'type' => 		'text', 
...
function gutentag_register_settings() {
	$settings_output = gutentag_get_settings();
	...
	register_setting($cmsmasters_option_name, $cmsmasters_option_name, 'gutentag_validate_options');
	
	
	if (!empty($settings_output['cmsmasters_page_sections'])) {
		foreach ($settings_output['cmsmasters_page_sections'] as $id => $title) {
			add_settings_section($id, $title, 'gutentag_section_fn', __FILE__);
		}
	}
	...
	if (!empty($settings_output['cmsmasters_page_fields'])) {
		foreach ($settings_output['cmsmasters_page_fields'] as $option) {
			gutentag_create_settings_field($option);
		}
	}
...
function gutentag_settings_scripts() {
	wp_enqueue_style('wp-jquery-ui-dialog');
	
	
	wp_enqueue_style('gutentag-settings-css', get_template_directory_uri() . '/framework/admin/settings/css/cmsmasters-theme-settings.css', array(), '1.0.0', 'screen');
	
	
	if (is_rtl()) {
		wp_enqueue_style('gutentag-settings-css-rtl', get_template_directory_uri() . '/framework/admin/settings/css/cmsmasters-theme-settings-rtl.css', array(), '1.0.0', 'screen');
	}
	...
	wp_enqueue_script('gutentag-settings-js', get_template_directory_uri() . '/framework/admin/settings/js/cmsmasters-theme-settings.js', array('jquery', 'farbtastic'), '1.0.0', true);
	
	
	$theme_style_name = '';
	
	if (CMSMASTERS_THEME_STYLE_COMPATIBILITY) {
		foreach (gutentag_all_theme_styles() as $theme_style) {
	...
	wp_localize_script('gutentag-settings-js', 'cmsmasters_setting', array( 
		'palettes' => 						implode(',', cmsmasters_color_picker_palettes()), 
		'remove' => 						esc_attr__('Remove', 'gutentag'), 
		'remove_sidebar' => 				esc_html__('Do you really want to remove this sidebar?', 'gutentag'), 
		'remove_google_web_font' => 		esc_html__('Do you really want to remove this google web font?', 'gutentag'), 
		'find' => 							esc_attr__('Find icons', 'gutentag'), 
		'remove_icon' => 					esc_html__('Do you really want to remove this social icon?', 'gutentag'), 
		'theme_uri' => 						get_template_directory_uri(), 
		'shortname' => 						'gutentag', 
		'theme_style_name' => 				$theme_style_name, 
		'done' => 							esc_html__('All theme settings were imported successfully.', 'gutentag'), 
		'fail' => 							esc_html__("Error on page!!!\nPlease reload page and try again.", 'gutentag'), 
		'done_reset_settings' => 			esc_html__('Theme style settings were reset to default.', 'gutentag'), 
	...
	wp_enqueue_script('gutentag-settings-js-toggle', get_template_directory_uri() . '/framework/admin/settings/js/cmsmasters-theme-settings-toggle.js', array('jquery'), '1.0.0', true);
	
	wp_localize_script('gutentag-settings-js-toggle', 'cmsmasters_settings', array( 
		'shortname' => 	'gutentag' 
	));
...
function gutentag_add_menu() {
	...
	$add_menu_p( 
		esc_attr__('Theme Settings', 'gutentag'), 
		esc_attr__('Theme Settings', 'gutentag'), 
		'manage_options', 
		'cmsmasters-settings', 
		'gutentag_settings_page_fn', 
		'' 
	);
	...
	$cmsmasters_settings_general = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme General Settings', 'gutentag'), 
		esc_attr__('General', 'gutentag'), 
		'manage_options', 
		'cmsmasters-settings', 
		'gutentag_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_general, 'gutentag_settings_scripts');
	
	
	$cmsmasters_settings_font = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme Fonts Settings', 'gutentag'), 
		esc_attr__('Fonts', 'gutentag'), 
		'manage_options', 
		'cmsmasters-settings' . '-font', 
		'gutentag_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_font, 'gutentag_settings_scripts');
	
	
	$cmsmasters_settings_color = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme Color Schemes', 'gutentag'), 
		esc_attr__('Colors', 'gutentag'), 
		'manage_options', 
		'cmsmasters-settings' . '-color', 
		'gutentag_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_color, 'gutentag_settings_scripts');
	
	
	$cmsmasters_settings_element = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme Elements Settings', 'gutentag'), 
		esc_attr__('Elements', 'gutentag'), 
		'manage_options', 
		'cmsmasters-settings' . '-element', 
		'gutentag_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_element, 'gutentag_settings_scripts');
	
	
	$cmsmasters_settings_single = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme Single Posts Settings', 'gutentag'), 
		esc_attr__('Single Posts', 'gutentag'), 
		'manage_options', 
		'cmsmasters-settings' . '-single', 
		'gutentag_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_single, 'gutentag_settings_scripts');
	
	
	$cmsmasters_settings_demo = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme Settings Import/Export', 'gutentag'), 
		esc_attr__('Import/Export', 'gutentag'), 
		'manage_options', 
		'cmsmasters-settings' . '-demo', 
		'gutentag_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_demo, 'gutentag_settings_scripts');
}


function gutentag_section_fn($desc) {
	$tab = gutentag_get_the_tab();
...
function gutentag_settings_page_fn() {
	$settings_output = gutentag_get_settings();
	$current_tab = gutentag_get_the_tab();
	
	echo '<div class="wrap">';
	
	gutentag_settings_page_header();
	...
	if ($current_tab != 'wpml') {
		echo '<form action="options.php" method="post" class="cmsmasters_admin_page' . (($current_tab == 'recaptcha') ? ' cmsmasters_admin_page_recaptcha' : '') . '">' . 
			'<p class="submit-top">' . 
				(($current_tab != 'import' && $current_tab != 'export' && $current_tab != 'theme_style') ? '<input name="submit-top" type="submit" class="button button-primary button-large" value="' . esc_attr__('Save Changes', 'gutentag') . '" />' : '') . 
			'</p>';
			...
			echo '<p class="submit">';
				...
				if ($current_tab != 'import' && $current_tab != 'export' && $current_tab != 'theme_style') {
					echo '<input name="submit" type="submit" class="button button-primary button-large" value="' . esc_attr__('Save Changes', 'gutentag') . '" />';
				} elseif ($current_tab == 'import') {
					echo '<input name="import" type="button" class="button button-primary button-large cmsmasters-demo-import" value="' . esc_attr__('Import Settings', 'gutentag') . '" />';
				} elseif ($current_tab == 'theme_style') {
					echo '<input name="apply-theme-style" type="submit" class="button button-primary button-large cmsmasters-apply-theme-style" value="' . esc_attr__('Apply This Style', 'gutentag') . '" />';
					echo '<input name="reset-settings" type="button" class="button button-large cmsmasters-reset-settings" value="' . esc_attr__('Reset Theme Style Settings to Default', 'gutentag') . '" />';
				}
...
function gutentag_form_field_fn($args = array()) {
	extract($args);
	...
	case 'select_theme_style':
		$theme_style_option = get_option('cmsmasters_gutentag_theme_style');
		
		echo '<select id="' . $id . '" class="select' . $field_class . '" name="cmsmasters_gutentag_theme_style">';
	...
	case 'upload':
		...
		echo '<div class="cmsmasters_upload_parent cmsmasters_select_parent">' . 
			'<input type="button" id="cmsmasters_upload_' . $id . '_button" class="cmsmasters_upload_button button button-large" value="' . esc_attr__('Choose Image', 'gutentag') . '" data-title="' . esc_attr__('Choose Image', 'gutentag') . '" data-button="' . esc_attr__('Insert Image', 'gutentag') . '" data-id="cmsmasters-media-select-frame-' . $id . '" data-classes="media-frame cmsmasters-media-select-frame' . ((!isset($description)) ? ' cmsmasters-frame-no-description' : '') . ((!isset($caption)) ? ' cmsmasters-frame-no-caption' : '') . ((!isset($align)) ? ' cmsmasters-frame-no-align' : '') . ((!isset($link)) ? ' cmsmasters-frame-no-link' : '') . ((!isset($size)) ? ' cmsmasters-frame-no-size' : '') . '" data-library="image" data-type="' . $frame . '"' . (($frame == 'post') ? ' data-state="insert"' : '') . ' data-multiple="' . $multiple . '" />' . 
			'<div class="cmsmasters_upload' . (($image != '') ? ' cmsmasters_db' : '') . '">' . 
				'<img src="' . (($image != '') ? $image : '') . '" class="cmsmasters_preview_image" />' . 
				'<a href="#" class="cmsmasters_upload_cancel admin-icon-remove" title="' . esc_attr__('Remove', 'gutentag') . '"></a>' . 
			'</div>' . 
	...
	case 'typorgaphy':
		...
		if ($system_font) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				foreach (gutentag_system_fonts_list() as $key => $value) {
					echo '<option value="' . esc_attr($key) . '"' . (($options[$id . '_system_font'] == $key) ? ' selected="selected"' : '') . '>' . esc_html($value) .'</option>';
				}
				
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_system_font">' . esc_html__('System Font', 'gutentag') . '</label>' . 
			'</div>';
		}
			...
			echo '<div class="cmsmasters_admin_block">' . 
				...
				$cmsmasters_fonts_list = cmsmasters_fonts_list();
					...
					if (!empty($cmsmasters_fonts_list['local'])) {
						echo '<optgroup label="' . esc_html__('Local Fonts', 'gutentag') . '">';
					...
					if (!empty($cmsmasters_fonts_list['web'])) {
						echo '<optgroup label="' . esc_html__('Google Web Fonts', 'gutentag') . '">';
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_google_font">' . esc_html__('Google Font', 'gutentag') . '</label>' . 
			'</div>';
		}
		...
		if ($font_size) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				' &nbsp; ' . 
				'<label for="' . $id . '_font_size_number">' . esc_html__('Font Size', 'gutentag') . ' <em>(' . esc_html__('in pixels', 'gutentag') . ')</em></label>' . 
			'</div>';
		}
		...
		if ($line_height) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				' &nbsp; ' . 
				'<label for="' . $id . '_line_height_number">' . esc_html__('Line Height', 'gutentag') . ' <em>(' . esc_html__('in pixels', 'gutentag') . ')</em></label>' . 
			'</div>';
		}
		...
		if ($font_weight) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_font_weight">' . esc_html__('Font Weight', 'gutentag') . '</label>' . 
			'</div>';
		}
		...
		if ($font_style) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_font_style">' . esc_html__('Font Style', 'gutentag') . '</label>' . 
			'</div>';
		}
		...
		if ($text_transform) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_text_transform">' . esc_html__('Text Transform', 'gutentag') . '</label>' . 
			'</div>';
		}
		...
		if ($text_decoration) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				foreach (gutentag_text_decoration_list() as $key => $value) {
					echo '<option value="' . esc_attr($key) . '"' . (($options[$id . '_text_decoration'] == $key) ? ' selected="selected"' : '') . '>' . esc_html($value) .'</option>';
				}
				
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_text_decoration">' . esc_html__('Text Decoration', 'gutentag') . '</label>' . 
			'</div>';
		}
	...
	case 'sidebar':
		...
		'<div class="sidebar_management">' . 
			'<p>' . 
				...
				'<input class="button" type="button" id="add_sidebar" value="' . esc_attr__('Add Sidebar', 'gutentag') . '" />' . 
			'</p>' . 
			'<div></div>' . 
			'<ul>';
	...
	case 'google_web_fonts':
		...
		'<div class="google_web_fonts_manager">' . 
				...
				if (isset($options[$id]) && is_array($options[$id])) {
					...
					foreach($options[$id] as $font) {
						...
						echo '<div class="google_web_fonts_manager_item">' . 
							'<input class="all-options" type="text" id="google_web_font_family" placeholder="' . esc_html__('Font Family (for example: Roboto Slab)', 'gutentag') . '" name="' . $cmsmasters_option_name . '[' . $id . '_-_' . $i . '_family]" value="' . esc_attr($font[1]) . '" />' . 
							'<input class="regular-text" type="text" id="google_web_font_style" placeholder="' . esc_html__('Font Style (for example: Roboto+Slab:100,300,400,700)', 'gutentag') . '" name="' . $cmsmasters_option_name . '[' . $id . '_-_' . $i . '_style]" value="' . esc_attr($font[0]) . '" />' . 
			...
			'<input class="button" type="button" id="add_google_web_font" value="' . esc_attr__('Add Font', 'gutentag') . '" data-font-family-label="' . esc_html__('Font Family (for example: Roboto Slab)', 'gutentag') . '" data-font-style-label="' . esc_html__('Font Style (for example: Roboto+Slab:100,300,400,700)', 'gutentag') . '" data-option-name="' . $cmsmasters_option_name . '[' . $id . '_-_" />' . 
		'</div>';
	...
	case 'social':
		...
		'<div class="icon_management">' . 
			'<p>' . 
				...
				'<input id="' . $id . '_button" class="cmsmasters_icon_choose_button button" type="button" value="' . esc_attr__('Choose icon', 'gutentag') . '" />' . 
				'<a href="#" class="cmsmasters_remove_icon admin-icon-remove" title="' . esc_attr__('Cancel changes', 'gutentag') . '"></a>' . 
			'</p>' . 
			'<span class="cl"><br /></span>' . 
			'<span class="icon_upload_link cmsmasters_dn">' . 
				'<label for="new_icon_color">' . 
					'<span class="cmsmasters_col_label">' . esc_html__('Icon Color', 'gutentag') . '</span>' . 
					'<div>' . 
				...
				'<label for="new_icon_hover">' . 
					'<span class="cmsmasters_col_label">' . esc_html__('Icon Hover Color', 'gutentag') . '<br /><em>' . esc_html__('if not selected - Icon Color will be used', 'gutentag') . '</em></span>' . 
					'<div>' . 
				...
				'<label for="new_icon_link">' . 
					...
					esc_html__('Icon Link', 'gutentag') . 
				'</label>' . 
				'<label for="new_icon_title">' . 
					'<input class="all-options" type="text" id="new_icon_title" /> ' . 
					esc_html__('Icon Title', 'gutentag') . 
				'</label>' . 
				'<label for="new_icon_target">' . 
					'<input type="checkbox" id="new_icon_target" value="true" /> ' . 
					esc_html__('Open link in a new tab/window?', 'gutentag') . 
				'</label>' . 
			'</span>' . 
			'<span class="cl"></span>' . 
			'<input class="button button-primary" type="button" id="add_icon" value="' . esc_attr__('Add Icon', 'gutentag') . '" />' . 
			'<input class="button button-primary" type="button" id="edit_icon" value="' . esc_attr__('Save Icon', 'gutentag') . '" />' . 
			'<ul>';
			...
			if (isset($options[$id]) && is_array($options[$id])) {
				foreach($options[$id] as $icon) {
					...
					echo '<li>' . 
						...
						'</div>' . 
						'<a href="#" class="icon_del admin-icon-remove" title="' . esc_attr__('Remove', 'gutentag') . '"></a> ' . 
...
function gutentag_validate_options($input) {
	$valid_input = array();
	...
	foreach ($options as $option) {
		...
		case 'text':
			...
			case 'numeric':
				...
				$valid_input[$option['id']] = (is_numeric($input[$option['id']])) ? $input[$option['id']] : esc_html__('Expecting a Numeric value!', 'gutentag');
				
				if (is_numeric($input[$option['id']]) == false) {
					add_settings_error(
						$option['id'],
						'gutentag' . '_txt_numeric_error',
						esc_html__('Expecting a Numeric value! Please fix.', 'gutentag'),
						'error'
					);
				}
			...
			case 'multinumeric':
				...
				if ($input[$option['id']] != '') {
					$valid_input[$option['id']] = (preg_match('/^-?\d+(?:,\s?-?\d+)*$/', $input[$option['id']]) == 1) ? $input[$option['id']] : esc_html__('Expecting comma separated numeric values', 'gutentag');
				} else {
				...
				if ($input[$option['id']] != '' && preg_match('/^-?\d+(?:,\s?-?\d+)*$/', $input[$option['id']]) != 1) {
					add_settings_error(
						$option['id'],
						'gutentag' . '_txt_multinumeric_error',
						esc_html__('Expecting comma separated numeric values! Please fix.','gutentag'),
						'error'
					);
				}
			...
			case 'email':
				...
				if ($input[$option['id']] != '') {
					$valid_input[$option['id']] = (is_email($input[$option['id']]) !== false) ? $input[$option['id']] : esc_html__('Invalid email! Please re-enter!', 'gutentag');
				} elseif ($input[$option['id']] == '') {
					$valid_input[$option['id']] = esc_html__('This setting field cannot be empty! Please enter a valid email address.', 'gutentag');
				}
				
				if (is_email($input[$option['id']]) == false || $input[$option['id']] == '') {
					add_settings_error(
						$option['id'],
						'gutentag' . '_txt_email_error',
						esc_html__('Please enter a valid email address.', 'gutentag'),
						'error'
					);
				}
		...
		case 'number':
			$input[$option['id']] = trim($input[$option['id']]);
			$valid_input[$option['id']] = (is_numeric($input[$option['id']])) ? $input[$option['id']] : esc_html__('Number!', 'gutentag');
...
function gutentag_show_msg($message, $msgclass = 'info') {
	echo '<div id="message" class="' . $msgclass . '">' . $message . '</div>';
}


function gutentag_admin_msgs() {
	global $pagenow;
...
function gutentag_add_global_options($reset = false) {
	...
	foreach (gutentag_all_color_schemes_list() as $key => $value) {
		array_push($cmsmasters_option_names, array( 
			'cmsmasters_options_' . 'gutentag' . CMSMASTERS_THEME_STYLE . '_color_' . $key, 
			gutentag_options_color_fields($key) 
		));
	}
		...
		if ($cmsmasters_option_name[1] !== '') {
			foreach ($cmsmasters_option_name[1] as $selected_option) {
				if ( 
					...
					$selected_option['id'] !== 'gutentag' . '_social_icons' && 
					$selected_option['id'] !== 'gutentag' . '_google_web_fonts' 
				) {
...
function gutentag_get_global_options() {
	...
	foreach (gutentag_all_color_schemes_list() as $key => $value) {
		array_push($cmsmasters_option_names, 'cmsmasters_options_' . 'gutentag' . CMSMASTERS_THEME_STYLE . '_color_' . $key);
	}
