...
function gutentag_breadcrumbs($sep_text = ' — ') {
	global $post;
	
	
	$homeLink = esc_url(home_url('/'));
	
	$homeText = esc_attr__('Home', 'gutentag');
	...
	if (CMSMASTERS_LEARNPRESS && learn_press_is_courses()) {
		...
		echo '<span>' . esc_html__('Search results for', 'gutentag') . ": '" . esc_html(get_search_query()) . "'</span>";
		...
		if (is_tax('post_format', 'post-format-gallery')) {
			echo '<span>' . esc_html_x('Galleries', 'post format archive title', 'gutentag') . '</span>';
		} elseif (is_tax('post_format', 'post-format-image')) {
			echo '<span>' . esc_html_x('Images', 'post format archive title', 'gutentag') . '</span>';
		} elseif (is_tax('post_format', 'post-format-video')) {
			echo '<span>' . esc_html_x('Videos', 'post format archive title', 'gutentag') . '</span>';
		} elseif (is_tax('post_format', 'post-format-audio')) {
			echo '<span>' . esc_html_x('Audio', 'post format archive title', 'gutentag') . '</span>';
		}
		...
		echo '<span>' . esc_html__('No breadcrumbs', 'gutentag') . '</span>';
	}
}
