...
if (!function_exists('gutentag_system_fonts_list')) {
	function gutentag_system_fonts_list() {
		$fonts = array( 
...
if (!function_exists('gutentag_text_decoration_list')) {
	function gutentag_text_decoration_list() {
		$list = array( 
			'none' => esc_html__('none', 'gutentag'), 
			'underline' => esc_html__('underline', 'gutentag'), 
			'overline' => esc_html__('overline', 'gutentag'), 
			'line-through' => esc_html__('line-through', 'gutentag'), 
		);
...
if (!function_exists('gutentag_custom_color_schemes_list')) {
	function gutentag_custom_color_schemes_list() {
		$list = array();
		
		
		return apply_filters('gutentag_custom_color_schemes_list_filter', $list);
	}
}
...
if (!function_exists('gutentag_locate_template')) {
	function gutentag_locate_template($template_names, $require_once = true, $load = true) {
		$located = '';
...
// Theme Style
define('CMSMASTERS_THEME_STYLE', (CMSMASTERS_THEME_STYLE_COMPATIBILITY && get_option('cmsmasters_gutentag_theme_style') ? get_option('cmsmasters_gutentag_theme_style') : ''));
...
// Visual Portfolio Plugin Compatibility
define('CMSMASTRES_VISUAL_PORTFOLIO', class_exists('Visual_Portfolio') ? true : false);

// GhostKit Plugin Compatibility
define('CMSMASTRES_GHOSTKIT', class_exists('GhostKit') ? true : false);
...
if (!function_exists('gutentag_all_theme_styles')) {
	function gutentag_all_theme_styles() {
		$out = array( 
			'Default|', 
			'Theme Style 1|1', 
			'Theme Style 2|2' 
...
if (!function_exists('gutentag_all_color_schemes_list')) {
	function gutentag_all_color_schemes_list() {
		$list = array( 
			'default' => 		esc_html__('Default', 'gutentag'), 
			'header' => 		esc_html__('Header', 'gutentag'), 
			'navigation' => 	esc_html__('Navigation', 'gutentag'), 
			'header_top' => 	esc_html__('Header Top', 'gutentag'), 
			'footer' => 		esc_html__('Footer', 'gutentag') 
		);
		
		
		$out = array_merge($list, gutentag_custom_color_schemes_list());
...
if (CMSMASTRES_VISUAL_PORTFOLIO) {
	require_once(get_template_directory() . '/visual-portfolio/cmsmasters-framework/theme-style' . CMSMASTERS_THEME_STYLE . '/cmsmasters-plugin-functions.php');
}

// Visual Portfolio functions
if (CMSMASTRES_GHOSTKIT) {
	require_once(get_template_directory() . '/ghostkit/cmsmasters-framework/theme-style' . CMSMASTERS_THEME_STYLE . '/cmsmasters-plugin-functions.php');
}



// Load Theme Local File
if (!function_exists('gutentag_load_theme_textdomain')) {
	function gutentag_load_theme_textdomain() {
		load_theme_textdomain('gutentag', CMSMASTERS_VARS . '/languages');
	}
}

// Load Theme Local File Action
if (!has_action('after_setup_theme', 'gutentag_load_theme_textdomain')) {
	add_action('after_setup_theme', 'gutentag_load_theme_textdomain');
}



// Framework Activation & Data Import
if (!function_exists('gutentag_theme_activation')) {
	function gutentag_theme_activation() {
		if (get_option('cmsmasters_active_theme') != 'gutentag') {
			add_option('cmsmasters_active_theme', 'gutentag', '', 'yes');
...
add_action('after_switch_theme', 'gutentag_theme_activation');



// Framework Deactivation
if (!function_exists('gutentag_theme_deactivation')) {
	function gutentag_theme_deactivation() {
		delete_option('cmsmasters_active_theme');
	}
}

add_action('switch_theme', 'gutentag_theme_deactivation');



// Plugin Activation Regenerate Styles
if (!function_exists('gutentag_plugin_activation')) {
	function gutentag_plugin_activation($plugin, $network_activation) {
...
add_action('activated_plugin', 'gutentag_plugin_activation', 10, 2);


if (!function_exists('gutentag_plugin_activation_regenerate')) {
	function gutentag_plugin_activation_regenerate() {
...
add_action('init', 'gutentag_plugin_activation_regenerate');

function gutentag_run_reinit_import_options($post_id, $key, $value) {
	if (!get_post_meta($post_id, 'cmsmasters_heading', true)) {
		$custom_post_meta_fields = gutentag_get_custom_all_meta_fields();
...
add_action('import_post_meta', 'gutentag_run_reinit_import_options', 10, 3);


function gutentag_writy_setup() {
	add_theme_support( 'align-wide' );
}

add_action( 'after_setup_theme', 'gutentag_writy_setup' );