...
<article id="post-<?php the_ID(); ?>" <?php post_class('cmsmasters_open_post'); ?>>
	...
	if ($cmsmasters_post_title == 'true') {
		gutentag_post_title_nolink(get_the_ID(), 'h2');
	}
	
	
	if (
		$cmsmasters_option['gutentag' . '_blog_post_date'] || 
		$cmsmasters_option['gutentag' . '_blog_post_author'] || 
		$cmsmasters_option['gutentag' . '_blog_post_cat'] 
	) {
		echo '<div class="cmsmasters_post_cont_info entry-meta">';
			
			gutentag_get_post_category(get_the_ID(), 'category', 'post');
			
			gutentag_get_post_author('post');

			gutentag_get_post_date('post');			
	...
	if ($cmsmasters_post_format == 'image') {
		...
		gutentag_post_type_image(get_the_ID(), $cmsmasters_post_image_link, $cmsmasters_image_thumb_size);
	} elseif ($cmsmasters_post_format == 'gallery') {
		$cmsmasters_post_images = explode(',', str_replace(' ', '', str_replace('img_', '', get_post_meta(get_the_ID(), 'cmsmasters_post_images', true))));
		
		gutentag_post_type_slider(get_the_ID(), $cmsmasters_post_images, $cmsmasters_image_thumb_size);
	} elseif ($cmsmasters_post_format == 'video') {
		$cmsmasters_post_video_type = get_post_meta(get_the_ID(), 'cmsmasters_post_video_type', true);
		$cmsmasters_post_video_link = get_post_meta(get_the_ID(), 'cmsmasters_post_video_link', true);
		$cmsmasters_post_video_links = get_post_meta(get_the_ID(), 'cmsmasters_post_video_links', true);
		
		gutentag_post_type_video(get_the_ID(), $cmsmasters_post_video_type, $cmsmasters_post_video_link, $cmsmasters_post_video_links, $cmsmasters_image_thumb_size);
	...
	if (get_the_content() != '') {
		echo '<div class="cmsmasters_post_content entry-content clearfix">';
...
if ($cmsmasters_post_author_box == 'true') {
	gutentag_author_box(esc_html__('About author', 'gutentag'), 'h2', 'h4');
}
...
if ($cmsmasters_post_more_posts != 'hide') {
	gutentag_related( 
		'h2', 
		esc_html__('More posts', 'gutentag'), 
		esc_html__('No posts found', 'gutentag'), 
		$cmsmasters_post_more_posts, 
		$tgsarray, 
		$cmsmasters_option['gutentag' . '_blog_more_posts_count'], 
		$cmsmasters_option['gutentag' . '_blog_more_posts_pause'], 
		'post' 
	);
}
