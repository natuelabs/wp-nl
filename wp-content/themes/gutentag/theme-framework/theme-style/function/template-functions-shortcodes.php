...
function gutentag_slider_post_heading($cmsmasters_id, $type = 'post', $tag = 'h1', $link_redirect = false, $link_url = false, $show = true, $link_target = false) { 
	if ($type == 'post') {
...
function gutentag_slider_post_exc_cont($type = 'post', $show = true) {
	if ($type == 'post') {
...
function gutentag_slider_post_check_exc_cont($type = 'post') {
	$exc = gutentag_slider_post_exc_cont($type, false);
...
function gutentag_get_slider_post_date($type = 'post', $show = true) {
	if ($type == 'post') {
...
function gutentag_get_slider_post_author($type = 'post', $show = true) {
	if ($type == 'post') {
		$out = '<span class="cmsmasters_slider_post_author">' . 
			esc_html__('By', 'gutentag') . ' ' . 
			'<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '" title="' . esc_attr__('Posts by', 'gutentag') . ' ' . esc_attr(get_the_author_meta('display_name')) . '" class="vcard author" rel="author"><span class="fn">' . esc_html(get_the_author_meta('display_name')) . '</span></a>' . 
		'</span>';
	} elseif ($type == 'project') {
...
function gutentag_get_slider_post_category($cmsmasters_id, $taxonomy, $type = 'post', $show = true) {
	$out = '';
...
function gutentag_slider_post_like($type = 'post', $show = true) {
	$out = '';
...
function gutentag_get_slider_post_comments($type = 'post', $show = true) {
	$out = '';
	
	
	if (comments_open()) {
		if ($type == 'post') {
			$out = gutentag_get_comments('cmsmasters_slider_post_comments');
		} elseif ($type == 'project') {
			$out = gutentag_get_comments('cmsmasters_slider_project_comments');
		}
	}
...
function gutentag_slider_post_more($cmsmasters_id, $show = true) {
	$cmsmasters_post_read_more = get_post_meta($cmsmasters_id, 'cmsmasters_post_read_more', true);
	
	
	if ($cmsmasters_post_read_more == '') {
		$cmsmasters_post_read_more = esc_attr__('Read More', 'gutentag');
	}
