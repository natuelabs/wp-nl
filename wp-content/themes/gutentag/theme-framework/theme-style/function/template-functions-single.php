...
function gutentag_prev_next_posts($order_cat = false, $tax_name = 'category') {
...
function cmsmasters_sharing_box($title_box = false, $tag = 'h3') {
	...
	return "<aside class=\"share_posts\">
		...
		<div class=\"share_posts_inner\">
			<a href=\"https://www.facebook.com/sharer/sharer.php?display=popup&u={$page_link}\">" . esc_html__('Facebook', 'gutentag') . "</a>
			<a href=\"https://twitter.com/intent/tweet?text=" . urlencode(html_entity_decode(sprintf(esc_attr__("Check out '%s' on %s website", 'gutentag'), $social_title, $website_name), ENT_QUOTES, 'UTF-8')) . "&url={$page_link}\">" . esc_html__('Twitter', 'gutentag') . "</a>
			<a href=\"https://pinterest.com/pin/create/button/?url={$page_link}&media={$pinterest_img}&description={$social_title}\">" . esc_html__('Pinterest', 'gutentag') . "</a>
		</div>
	</aside>
";
}
...
/* Get Sharing Box Function */
function gutentag_sharing_box($title_box = false, $tag = 'h3') {
...
function gutentag_author_box($title_box = false, $tag = 'h3', $author_tag = 'h4') {
	...
	if ($user_description) {
		...
		echo '<figure class="about_author_avatar">' . 
			get_avatar($user_email, 230, get_option('avatar_default')) . 
		'</figure>' . 
		...
		if ($user_url) {
			echo '<a href="' . esc_url($user_url) . '" title="' . esc_attr(get_the_author()) . ' ' . esc_attr__('website', 'gutentag') . '" target="_blank">' . esc_html($user_url) . '</a>';
		}
...
function gutentag_related($tag = 'h3', $title = '', $no_title = '', $box_type = false, $tgsarray = null, $items_number = 5, $pause_time = 5, $type = 'post', $taxonomy = null) {
	if ( 
		...
		if ($r_query->have_posts()) {
			echo "<aside class=\"cmsmasters_single_slider\">" . 
				"<" . esc_html($tag) . " class=\"cmsmasters_single_slider_title\">" . 
					($title != '' ? esc_html($title) : esc_html__('More items', 'gutentag')) . 
				"</" . esc_html($tag) . ">" . 
						...
						while ($r_query->have_posts()) : $r_query->the_post();
							echo "<div class=\"cmsmasters_owl_slider_item cmsmasters_single_slider_item\">
								<div class=\"cmsmasters_single_slider_item_outer\">";
									...
									gutentag_thumb(get_the_ID(), 'cmsmasters-project-thumb', true, false, true, false, true, true, false, false, false, 'cmsmasters_theme_icon_image');
...
function gutentag_author_avatar($template_type = 'page') {
...
function gutentag_get_pings($id, $tag = 'h3') {
	$out = '';
	...
	if (sizeof($pings) > 0) {
		$out .= '<aside class="cmsmasters_pings_list">' . "\n" .
			'<' . esc_html($tag) . '>' . esc_html__('Trackbacks and Pingbacks', 'gutentag') . '</' . esc_html($tag) . '>' . "\n" .
