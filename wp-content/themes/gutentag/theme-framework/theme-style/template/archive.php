...
<article id="post-<?php the_ID(); ?>" <?php post_class('cmsmasters_archive_type'); ?>>
	...
	if (!post_password_required() && has_post_thumbnail()) {
		echo '<div class="cmsmasters_archive_item_img_wrap">';
			gutentag_thumb(get_the_ID(), 'cmsmasters-square-thumb');
		echo '</div>';
	}
	?>
	<div class="cmsmasters_archive_item_cont_wrap">
			...
			echo '<div class="cmsmasters_archive_item_info entry-meta">';
				...
				if ($current_tax != '') {
					echo '<span class="cmsmasters_archive_item_category">' . 
						gutentag_get_the_category_list(get_the_ID(), $current_tax, ' ') . 
					'</span>';
				}
				...
				if (get_post_type() == 'post') {
					echo '<span class="cmsmasters_archive_item_user_name">' . 
						'<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '" rel="author" title="' . esc_attr__('Posts by', 'gutentag') . ' ' . get_the_author_meta('display_name') . '">' . get_the_author_meta('display_name') . '</a>' . 
					'</span>';
				}
		...
		if (gutentag_excerpt(55, false) != '') {
			echo cmsmasters_divpdel('<div class="cmsmasters_archive_item_content entry-content">' . "\n" . 
				wpautop(gutentag_excerpt(55, false)) . 
			'</div>' . "\n");
		}

		echo '<div class="cmsmasters_archive_item_link"><a class="cmsmasters-icon-custom-right-arrow" href="' . get_the_permalink() . '"></a></div>';
		?>
	</div>
</article>