...
class Custom_Category_Templates {
	...
	function category_template( $template ) {
		...
		$id = $category->term_id;
		$tmpl = isset($category_templates[$id]) ? locate_template( $category_templates[$id] ) : '';
	...
	function add_template_option() { ?>
		<div class="form-field">
			<label for="template"><?php _e('Template','jmax'); ?></label>
			<select name="template" id="template" class="postform">
				<option value='default'><?php _e('Default Template','jmax'); ?></option>
				<?php $this->category_templates_dropdown() ?>
			</select>
			<!--<p class="description"><?php _e('Template file to render category archive.','jmax'); ?></p>-->
		</div>
	<?php }
