...
class Sidebar_Login {
	...
	public function enqueue() {
		...
		// Register BLOCK UI
		wp_register_script( 'jquery-blockui', get_template_directory_uri().'/inc/jmax-widgets/sidebar-login/assets/js/blockui.min.js', array( 'jquery' ), '2.57', $js_in_footer );

		// Enqueue Sidebar Login JS
		wp_enqueue_script( 'sidebar-login', get_template_directory_uri().'/inc/jmax-widgets/sidebar-login/assets/js/sidebar-login' . $suffix . '.js', array( 'jquery', 'jquery-blockui' ), $this->version, $js_in_footer );

		// Enqueue Styles
		if ( apply_filters( 'sidebar_login_include_css', true ) ) {
			wp_enqueue_style( 'sidebar-login', get_template_directory_uri().'/inc/jmax-widgets/sidebar-login/assets/css/sidebar-login.css', '', $this->version );
		}

		// Pass variables
		$sidebar_login_params = array(
			'ajax_url'         => $this->ajax_url(),
			'force_ssl_login'  => force_ssl_admin() ? 1 : 0,
